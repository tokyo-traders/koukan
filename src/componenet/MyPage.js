...
export default function MyPage() {
  ...
  const itemList = useCallback(() => navigate("/MyPage"), [navigate]);
  const postList = useCallback(() => navigate("/MyPage/postList"), [navigate]);
  const offerList = useCallback(
    () => navigate("/MyPage/offered-items/"),
    [navigate]
  );
  const pendingTrade = useCallback(
          ...
          <Typography>
            <Rating
              ...
              value={
                user?.rating !== 0
                  ? (
                      Math.round(user?.reputation_rating * 10) /
                      10 /
                      user?.total_review
                    ).toFixed(1)
                  : 0
